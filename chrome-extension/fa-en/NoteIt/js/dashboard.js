...
window.onload = function() {
    ...
    $("#languege").click(function(){
    	chrome.storage.local.get("sign", function(data) {
    		if(data.sign!=null && data.sign['languege']!=null){
    			var set={};
    			set['signin']=data.sign['signin'];
    			set['name']=data.sign['name'];
    			set['access_token']=data.sign['access_token'];
    			if(data.sign['languege']=='en'){
    				set['languege']='fa';
    				$("#img_languege").attr('src',chrome.extension.getURL("img/icons/fa.png"));
    			}else{
    				set['languege']='en';
    				$("#img_languege").attr('src',chrome.extension.getURL("img/icons/en.png"));
    			}
    			setSignin(set);
    			updatelist();
    		}
...
function init(){
	chrome.storage.local.get("sign", function(data) {
	  	if(data.sign!=null && data.sign['signin']!=null && data.sign['languege']){
	  		$("#user-name").text(data.sign['name']);
	  		$("#img_languege").attr('src',chrome.extension.getURL("img/icons/"+data.sign['languege']+".png"));
		}
	});
...
function updatelist(){
    ...
    chrome.storage.local.get("sign", function(data) {
      if(data !=null && data.sign!=null){
      	if(data.sign['languege']=='en'){
			title='latest updated articles';
			$("#title").text('...loading Articles');
		}else{
			title='آخرین مقاله های بروز شده';
			$("#title").text('دریافت مقاله ها...');
		}
        $.ajax({
	...
	chrome.storage.local.get("sign", function(data) {
      if(data !=null && data.sign!=null){
      	if(data.sign['languege']=='en'){
			...
			title="آخرین متفرقه ها";
			$("#title").text('در حال بارگذاری متفرقه ها...');
		}
        $.ajax({
