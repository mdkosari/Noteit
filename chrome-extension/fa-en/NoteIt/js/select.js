...
NoteIt.Selector.mouseup = function(e){
        ...
        function(request, sender, sendResponse) {
          if(request.type == "showOptionBox") {
            ...
            if (selection && selection.rangeCount > 0){ // Add new node only if something is actually selected
              ...
              $('#'+id).webuiPopover({placement:'auto',content: content ,padding:false ,margin:false ,width:'40' ,height:'50' ,closeable:false, trigger: "click"});
            }
  ...
  function getArticles(range,id,selection,style,text){
    ...
    updateArticle(id,add,addFa,style,text);
  }

  function updateArticle(id,add,addFa,style,text){
    chrome.storage.local.get("sign", function(local) {
      if(local !=null && local.sign!=null){
        $.ajax({
          url:"https://noteit.pythonanywhere.com/note/api/articles/",
          type:"get",
          async:"true",
          headers: {  "Authorization": "Bearer "+local.sign['access_token']},
          data:{
          },
          success:function(data){
            ...
            $("img").click(function(){
              ...
              if($("#addarticle"+id).length){
                ...
                if(local.sign['languege']=='en')
                  $(add).insertBefore("#body"+id);
                else
                  $(addFa).insertBefore("#body"+id);
